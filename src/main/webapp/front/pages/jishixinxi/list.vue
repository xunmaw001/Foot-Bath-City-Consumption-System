<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<input v-model="searchForm.jishimingcheng" type="text" placeholder="技师名称" ></input>
			</view>
        ...
        <view @tap="onDetailTap(product)" v-if="index%6==0" class="list-item" :style='{"padding":"20rpx 30rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"14rpx 0","borderColor":"rgba(22, 113, 193, 1)","backgroundColor":"rgba(255, 255, 255, 1)","borderRadius":"60rpx 0 60rpx 0","borderWidth":"4rpx","width":"100%","borderStyle":"solid","height":"auto"}' style="display: flex;">
                                                                                          <image :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"rgba(22, 113, 193, 1)","backgroundColor":"rgba(255,0,0,0)","borderRadius":"0","borderWidth":"4rpx","width":"240rpx","borderStyle":"solid","height":"180rpx"}' mode="aspectFill" :src="product.jishitouxiang?baseUrl+product.jishitouxiang.split(',')[0]:''"></image>
                                                                                                                                                                          <view class="list-item-body" style="flex: 1;">
            <view :style='{"padding":"20rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"40rpx","fontSize":"28rpx","borderStyle":"solid"}' class="title">{{product.jishihao}}</view>
                                                                                                                                                                                                                                                                      </view>
                                                                                                                                                                                                                  <view style="display: flex;justify-content: space-between;">
            <text v-if="userid && isAuth('jishixinxi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="!userid && isAuthFront('jishixinxi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="userid && isAuth('jishixinxi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
            <text v-if="!userid && isAuthFront('jishixinxi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
          </view>
        </view>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.jishimingcheng="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.jishimingcheng){
					params['jishimingcheng'] = '%' + this.searchForm.jishimingcheng + '%'
				}
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`jishixinxi`, params);
                } else {
                    res = await this.$api.list(`jishixinxi`, params);
                }
				// 如果是第一页数据置空
			...
			async search(){
				...
				if(this.searchForm.jishimingcheng){
					searchForm['jishimingcheng'] = '%' + this.searchForm.jishimingcheng + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`jishixinxi`, searchForm);
                } else {
                    res = await this.$api.list(`jishixinxi`, searchForm);
                }
				// 如果是第一页数据置空
