<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="会员周期" prop="huiyuanzhouqi">
          <el-select :disabled="ro.huiyuanzhouqi" @change="huiyuanzhouqiChange" v-model="ruleForm.huiyuanzhouqi" placeholder="请选择会员周期">
            <el-option
                v-for="(item,index) in huiyuanzhouqiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.huiyuanzhouqi" label="会员周期" prop="huiyuanzhouqi">
              <el-input v-model="ruleForm.huiyuanzhouqi" 
                placeholder="会员周期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.yonghuxingming!=''&&json.yonghuxingming){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/huiyuanka/huiyuanzhouqi`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.huiyuanzhouqiOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    huiyuanzhouqiChange () {
      this.$http({
        url: `follow/huiyuanka/huiyuanzhouqi?columnValue=`+ this.ruleForm.huiyuanzhouqi,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.huiyuanjiage){
                this.ruleForm.huiyuanjiage = data.data.huiyuanjiage
              }
              if(data.data.huiyuanyouhui){
                this.ruleForm.huiyuanyouhui = data.data.huiyuanyouhui
              }
        } else {
          this.$message.error(data.msg);
        }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `huiyuankashenqing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.huiyuankashenqingCrossAddOrUpdateFlag = false;
						 this.parent.search();
